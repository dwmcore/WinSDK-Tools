...
extern FIFO_TX_BLOCK Sdm845TransferContext;
...

{
    ...
    if (Sdm845TransferContext.AvailableBytes == 0) {
        Sdm845TransferContext.PtrToFifoBuffer = (UCHAR *)Sdm845TransferContext.FifoBuffer;
        ...
        if (AvailableBytes > MAX_RX_FIFO_SIZE) {
            SDM845v2ReinitializePort(Port);
            return UartNotReady;
        }

        Sdm845TransferContext.AvailableBytes = AvailableBytes;
        for (Index = 0; Index < WordsToRead; Index += 1) {
            ...
            Sdm845TransferContext.FifoBuffer[0 + ArrayIndex] = (UCHAR)(RxFifo >>  0);
            Sdm845TransferContext.FifoBuffer[1 + ArrayIndex] = (UCHAR)(RxFifo >>  8);
            Sdm845TransferContext.FifoBuffer[2 + ArrayIndex] = (UCHAR)(RxFifo >> 16);
            Sdm845TransferContext.FifoBuffer[3 + ArrayIndex] = (UCHAR)(RxFifo >> 24);
            ArrayIndex += 4;
        }
        ...
        if (PartialBytesToRead > 0) {
            ...
            for (Index = 0; Index < PartialBytesToRead; Index += 1) {
                Sdm845TransferContext.FifoBuffer[ArrayIndex] = (UCHAR)(RxFifo >> Index * 8);
                ArrayIndex += 1;
            }
        }
    }
    ...
    if (Sdm845TransferContext.AvailableBytes != 0) {
        *Byte = *Sdm845TransferContext.PtrToFifoBuffer;
        Sdm845TransferContext.PtrToFifoBuffer += 1;
        Sdm845TransferContext.AvailableBytes -= 1;
        return UartSuccess;
    }
